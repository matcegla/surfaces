...
CameraFPS::CameraFPS(glm::vec3 pos)
    : pos(pos), up(0.0f, 1.0f, 0.0f), front(1.0f, 0.0f, 0.0f), yaw(0.0f),
      pitch(0.0f), last(0.0f, 0.0f), init(false) {}
...
void CameraFPS::handleCursorPos(float x, float y) {
  ...
  if (pitch < -89.0f)
    pitch = -89.0f;
  front = glm::normalize(
      glm::vec3(cosf(glm::radians(pitch)) * cosf(glm::radians(yaw)),
...
glm::mat4 CameraFPS::viewMatrix() { return glm::lookAt(pos, pos + front, up); }
